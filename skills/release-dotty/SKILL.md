...
**Decision authority.** Autonomous: computing the tag, running the refuse checks, discovering consumers. Escalate: the consumer bump PRs touch every consumer's gitleaks hook chain — operator approval before invoking this skill for real, not just before opening the PRs. Pushing dotty's own tag is never autonomous, full stop (LEX-757): a session used to push it under whatever credentials happened to be active, which named no real identity boundary — the script now computes and validates the tag, then hands her the one push as a deliberate process rule, not a GitHub-enforced one.
...
2. **Re-entry** — if `HEAD` already carries a date-based tag locally *and* that tag is confirmed on origin, skip straight to the bump phase for consumers still lagging it. The Release itself is still checked and cut independently of the tag either way (`gh release view || gh release create`) — a failure between the two must never leave a tag with no Release. If `HEAD` carries the tag locally but it isn't on origin yet, the script reprints the push command and exits — it never checks the Release or touches consumers until she's pushed.
3. **Due check** — if a tag already exists and no export changed since it, exit cleanly: nothing to release.
4. **Tag computation and handoff** — first tag of a UTC day is `vYYYY.MM.DD`; later ones append `-N`, N the next integer (compared numerically) after the highest existing suffix for that date, bare tag counting as `-1`. Refuses on an already-existing computed tag (checked against origin, not local state — a local cache can be stale) or a same-day tag that doesn't fit the scheme. Once computed and validated, the script creates the tag *locally only* and prints the exact `git push` command for her to run — it never pushes dotty's own tag itself (LEX-757); re-running the script after she's pushed resumes at step 2's re-entry path.
