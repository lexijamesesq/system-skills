...
for _profile in personal professional; do
  ...
  if [ -n "$PLUGIN_JSON" ] && command -v jq >/dev/null 2>&1; then
    version=$(printf '%s' "$PLUGIN_JSON" | jq -r '.[] | select(.id == "estate-hooks@core") | .version // empty' 2>/dev/null)
    enabled=$(printf '%s' "$PLUGIN_JSON" | jq -r '.[] | select(.id == "estate-hooks@core") | .enabled // false' 2>/dev/null)
    install_path=$(printf '%s' "$PLUGIN_JSON" | jq -r '.[] | select(.id == "estate-hooks@core") | .installPath // empty' 2>/dev/null)
